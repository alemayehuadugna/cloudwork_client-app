...
class CanceledJob implements UseCase<List<JobEntity>, PaginationParams> {
  final JobRepository repository;

  CanceledJob({required this.repository});
  ...
  Future<Either<Failure, List<JobEntity>>> call(PaginationParams params) async {
    ...
    Either<Failure, List<JobEntity>> jobs = await repository.cancelJobs(params);
    jobs.fold((failure) {
      return Left(failure);
    }, (jobs) {
      jobs.forEach(
