...
class PendingJob implements UseCase<List<JobEntity>, PaginationParams> {
  final JobRepository repository;

  PendingJob({required this.repository});
  ...
  Future<Either<Failure, List<JobEntity>>> call(PaginationParams params) async {
    ...
    Either<Failure, List<JobEntity>> jobs = await repository.pendingJobs(params);
    jobs.fold((failure) {
      return Left(failure);
    }, (jobs) {
      jobs.forEach(
